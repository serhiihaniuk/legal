...
/**
 * Write a generated file via temp file + rename so a mid-write interruption
 * (disk full, permission error, process kill) never leaves a truncated or
 * partially written file at `filePath`; the previous content (or nothing, if
 * this is a first write) survives unless the rename itself commits.
...
export async function generateRegistry({
  ...
  await Promise.all([
    ...
    writeGeneratedFile(referenceOutputPath, referenceSource),
  ])
  return {
    outputPath,
    referenceOutputPath,
