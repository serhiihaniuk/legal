...
- Primary product surfaces: wielostronicowy, redakcyjny workspace prawny z indeksem nauki, mapą wiedzy, panelem szczegółów, planem nauki i porównawczym case study z osią czasu
...
- Indeks rozdziałów KPA grupuje pozycje według `Dział`, pokazuje pełne wielowierszowe nazwy bez obcinania oraz jawne metadane `zakres artykułów · liczba artykułów`. Długi indeks ma widoczny, przewidywalny własny scroll.
...
- **Moduł KPA:** zaczyna od odpowiedzi „co to jest, jakie pytanie rozwiązuje i gdzie znajduje się w całej procedurze”; następnie pokazuje konkretny mechanizm instytucji, kluczowe pojęcia i granicę wobec `lex specialis`; osobna sekcja daje komentarz `artykuł po artykule` z widocznym streszczeniem każdego przepisu, objaśnieniem paragrafów/punktów, skutkiem prawnym i znaczeniem dla sprawy cudzoziemca; moduł kończy pełny przykład oraz typowe błędy. Checklist nie zastępuje materiału.
- **Artykuł KPA:** najpierw konkretne objaśnienie tego, co dana norma reguluje i ustanawia; następnie objaśnienie każdego paragrafu/punktu, skutek prawny i znaczenie w sprawie cudzoziemca; dopiero potem polski tekst źródłowy, miejsce w kodeksie i dokładny link do oficjalnego tekstu. Nie zastępujemy komentarza ogólną sekcją „jak czytać”.
...
- **Dokument:** kto wydaje lub przygotowuje; kiedy jest potrzebny; co dowodzi i czego nie dowodzi; wymagane elementy, forma i ważność; sposób uzyskania i kontroli; typowe braki; reakcja na wezwanie; przykład; zwarte źródła.
- **Case:** pełny przewodnik po jednej modelowej sprawie, a nie skrócony przykład. Lewa nawigacja wybiera rodzinę sprawy (`Karta pobytu`, `Pobyt stały`, `Rezydent UE`), a górne zakładki pokazują wyłącznie podsprawy aktywnej rodziny (np. `CUKR`, pobyt czasowy i praca, Blue Card, studia, działalność, rodzina i inne podstawy). Każda podsprawa zmienia fakty modelowe, wynik, warunki, dowody, terminy, ryzyka i przebieg; nie udajemy, że jedna osoba spełnia wszystkie podstawy naraz.
- **Case — kolejność treści:** orientacja i wynik → fakty modelowe i granice → wybór podstawy → 6–8 opisanych etapów od kwalifikacji do decyzji i obowiązków po decyzji → rejestr dokumentów → terminy i negatywne gałęzie → źródła. Każdy etap zaczyna się od prozowego wyjaśnienia mechanizmu, a dopiero potem pokazuje dowody, ryzyka i działania.
- **Case — powiązane materiały:** linki wewnętrzne są wyraźnymi kontrolkami nawigacyjnymi z opisem, po co otworzyć materiał. Prowadzą tylko do istniejących głębokich adresów mapy, modułów KPA, artykułów KPA albo katalogu dokumentów; nie wplatamy ich w środek akapitów.
