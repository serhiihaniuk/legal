...
- Font loading (2026-09-10): preload one complete Geist variable font in the document head. `app/fonts.css` uses `font-display: optional`. Separate Cyrillic and Latin files caused a baseline shift when punctuation and spaces loaded after Ukrainian letters. Slow first visits may retain the system font; do not force a late replacement that changes line wrapping. The font source and license live in `app/assets/fonts/`.
- Root and homepage CSS use explicit stylesheet links through React Router. In development, embedding critical CSS in the streamed HTML allowed the centered hero to paint before its remaining content arrived. Cold-cache checks at 390 × 844 and 1440 × 1000 with 400 ms latency and 1 Mbps download showed no layout shifts after switching to stylesheet links and the combined font.
...
- Map contents (2026-09-06): use subject chapters, not a mandatory chronology. Explain the case sequence separately through examples. Evidence, procedural rights and remedies can matter at several moments. The contents order is reasoning, present status, residence routes, employment, evidence, filing, proceedings, decisions and protection.
- Map chapter descriptions explain what the chapter contains before its topic list is expanded. Counts come from the same registry as desktop and mobile navigation. Preserve existing `#stage-*` URLs as chapter anchors. Every published topic has one primary chapter; retired routes have explicit compatibility destinations.
...
- The map uses the shared left index: the return control comes first, followed by chapters with the active chapter's topics nested beneath it. The article header shows its primary chapter, without the older subject-group badge, competing parent path or repeated stage question and outcome. Existing cross-topic links remain available as related reading.
...
- Map articles may include completed document examples where a real fragment or filled table explains the subject. Use the same `DocumentWorkedExample` and `DocumentSample` patterns as document guides. Keep fictional labels, facts, specimen, reasoning and conclusion together. Literal letter specimens retain their own attachment numbering; explanatory prose still uses typed legal references.
...
- Map articles (2026-09-06): migrated topics use an introduction and authored sections with stable IDs, subject-specific headings and connected paragraphs. The on-page contents derives from these sections. Reject duplicate/invalid IDs, empty sections and blank paragraphs before publication. Keep the old guide contract only for topics awaiting an individual rewrite; do not generate empty legacy fields for a new article.
...
- Deadline registers (2026-09-05): group deadlines by the existing case stages along a quiet SVG connector. Mark stages, not elapsed time. Each entry shows the action and period first, followed by the trigger, consequence, and legal source in one reading order. Reuse the entry component inside stage disclosures. Preserve every authored field; do not turn legal processing periods into promised completion dates. Keep all core text visible and stack the header on phones.

- Documents and evidence (2026-09-05): distinguish individual documents, evidence sets, and working records. Keep simple entries concise. Complex topics use authored explanation sections and completed examples with facts, reasoning, and a conclusion. Travel history includes dated evidence and explicitly unresolved facts. The catalog groups real links by subject and describes each entry. Case backlinks come from explicit document references in the current guides. Related contexts may be disclosed; core explanations remain visible. Page composition, examples, checks, contexts, sources, and adjacent navigation have separate components. Category selection is stored in the URL.

- Case-guide refinement (2026-09-05): preserve the document-first hierarchy of `Karta pobytu`, `Pobyt stały`, and `Rezydent UE`, with the existing subtypes inside each family. Improve the current full guide in place. Keep its facts, choice of basis, stages, document and deadline registers, alternatives, and sources in their existing reading order. Component extraction must preserve that experience. Shortcuts may take readers directly to the stages or registers; do not replace the guide with a general reference article or flatten document families into a list of purposes.
- Case-stage presentation (2026-09-05): use a numbered heading, a question in sentence case, quiet actor metadata, explanatory paragraphs, an ordinary ordered list, and a short outcome. Group documents, deadlines, and complications in matching independent shadcn disclosures. Keep related reading in a quiet footer. Avoid stacking metadata tables, bordered action rows, filled accordion bars, and separate deadline callouts. Document titles link to authored document-guide IDs; legal-basis references keep their provision destinations.
...
- Map overview (2026-09-06): a standalone explanatory article introduces the covered subjects, explains their connections and includes a completed illustrative example before the chapter contents. Avoid generic icon legends or instructions as substitutes for explanation. The internal subject groups do not create a second visible hierarchy. Chapter navigation opens the selected list, retains its URL anchor and works with browser Back.
...
Authored typed references rendered through the `app/components/references` public API use the shared shadcn `HoverCard` as a compact orientation aid. `LegalLink` and `LegalText` default to `prose`; reference/source rows use `reference-section`, and a provision reader uses `provision-page`. These contexts record semantics but deliberately share one quiet inherited-color link treatment. The trigger remains the real internal `Link` or external HTTPS anchor: pointer hover and keyboard focus may open the card, while click, `_blank`, and touch navigation are never intercepted. Do not add previews to ordinary navigation links, source-reader controls, or quoted statutory text that was not authored as a typed reference.
...
- Distinguish articles, paragraphs, and annexes in navigation. Use the provision locator as the reader heading and put the act title below it. Explanations of individual parts use the full text width.
