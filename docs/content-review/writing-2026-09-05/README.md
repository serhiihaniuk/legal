...
| Every case has appropriate document coverage | For all nine guides, compare each stage and register with official requirements. Record mandatory, conditional, and working documents; verify typed links in both directions. | Work and Blue Card reviewed in batches 15–16; seven complete routes remain pending. |
| Complex documents have useful explanations and examples | Read every document guide. Explain fields, what the document proves, what it leaves open, and a completed discrepancy example where needed. Simple documents need no artificial expansion. | All 58 guides have current writing acceptance after batch 16, including the new Blue Card notification. Examples include filled working records and selected fictional fragments; original-form and interface fidelity limits remain recorded in the batches. |
...
After batch 16, 84 of 1,772 inventoried files have current writing acceptance, including all 58 document guides and the complete work and Blue Card guides. The corpus audit must address the university and family annex page ranges, the family form's sponsor-status choices and the split words in extracted Polish text recorded in batches 09 and 13.

1. Remaining seven case routes, beginning with student and its distinct programme, evidence and financial requirements.
