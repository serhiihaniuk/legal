...
| Every case has appropriate document coverage | For all nine guides, compare each stage and register with official requirements. Record mandatory, conditional, and working documents; verify typed links in both directions. | Work, Blue Card, student, business, family, permanent residence and EU resident reviewed in batches 15–21; two complete routes remain pending. |
| Complex documents have useful explanations and examples | Read every document guide. Explain fields, what the document proves, what it leaves open, and a completed discrepancy example where needed. Simple documents need no artificial expansion. | All 61 guides have current writing acceptance after batch 20, including Karta Polaka. Examples include filled working records and selected fictional fragments; original-form and interface fidelity limits remain recorded in the batches. |
...
After batch 21, 107 of 1,790 inventoried files have current writing acceptance, including all 61 document guides and seven complete case guides. The corpus audit must address the university and family annex page ranges, the family form's sponsor-status choices and the split words in extracted Polish text recorded in batches 09 and 13.
...
1. Remaining two case routes: CUKR and other purposes. Continue with CUKR's registry conditions, filing, personal actions and the distinction between status before and after receiving the card.
