...
Batch 67 completes graduate document actions and adds a focused diploma guide. Its bank evidence now uses the graduate regulation rather than silently copying student issuer rules. Current policy, omitted extension, insurer confirmation and separate payment are identifiable records. Initial diploma, correspondence, funds and lease are reused in the reply. Eight files passed independent bounded review; full application and responsive browser checks passed. Permanent residence and earlier housing/insurance destinations remain open.

Batches 68–69 complete the planned permanent-residence list migration and reconcile earlier lease/private-policy destinations. The initial lease scan, requested original and first employment-contract submission are explicit. A matching Polish response explains settlement intent. Student, business and resident selected records retain their existing dates and amounts. Thirteen authored files passed independent bounded review and the full application checks passed. This resolves the named migration items above; it does not accept every unchanged alternative, map article, law commentary or source layout.
