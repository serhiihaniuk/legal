...
Latest implementation note, batch 47: all five orientation topics (`start-case`, `legal-anatomy`, `special-vs-kpa`, `two-keys`, `source-check`) now have authored explanations with completed examples. Their routes and primary chapter are unchanged. Next are `person-status` and the current-stay topics. The findings below preserve the original pre-migration audit; completed changes are recorded in the numbered batches.
