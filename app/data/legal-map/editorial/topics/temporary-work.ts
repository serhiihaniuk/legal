  ...
  defineKnowledgeUnit({
    ...
    body: {
      ...
      documents: [
        ...
        law.text`Письмові uzgodnienia агенції та користувача за ${temporaryArticle("art. 9")}: вид роботи, кваліфікації, строк, час, місце та розподіл визначених обов'язків. Це домовленості двох суб'єктів, окремі від договору працівника.`,
        law.text`Świadectwo pracy агенції з користувачами та періодами за ${temporaryArticle("art. 18a ust. 1a")}, а для цивільної роботи відповідне zaświadczenie за ${temporaryArticle("art. 25a ust. 2")}: різні документи про минулі періоди.`,
        law.text`Ewidencja користувача за ${temporaryArticle("art. 14a")}: його власний запис періодів роботи кожної направленої людини, з датами початку та закінчення.`,
        ...
        sections: [
          ...
          },
          {
            ...
            paragraphs: [
              ...
              law.text`До відповідної заяви агенції додають документ, складений pracodawcą użytkownikiem, який підтверджує погоджене направлення іноземця. Для звичайного дозволу це ${law.external("§ 2 pkt 2", attachmentsUrl)}, для oświadczenia ${law.external("§ 7 pkt 2", attachmentsUrl)} розпорядження про додатки. Сезонні заяви мають відповідні правила ${law.external("§ 5 pkt 4 і § 6 ust. 1 pkt 1", attachmentsUrl)}. Таке підтвердження користувача не тотожне всім письмовим узгодженням підприємств за ${temporaryArticle("art. 9")}, трудовому договору або вже виданому дозволу. Воно потрібне для відповідної процедури, якої у наведеному прикладі немає.`,
