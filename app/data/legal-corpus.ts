import { getEdition, listDocuments, listEditions } from "~/data/legal-library"
 ...
 */
export function getLegalSourceManifest(
  documentIdOrEditionId: string | undefined
) {
  if (!documentIdOrEditionId) return undefined

  for (const document of listDocuments()) {
    const editionId = document.editionIds.find(
      (candidate) => candidate === documentIdOrEditionId
    )
    if (editionId) return getEdition(document.id, editionId)?.manifest
  }

  const document = listDocuments().find(
    (candidate) => candidate.id === documentIdOrEditionId
  )
  if (document)
    return getEdition(document.id, document.currentEditionId)?.manifest

  for (const candidate of listDocuments()) {
    const edition = listEditions(candidate.id).find(
      (item) => item.editionId === documentIdOrEditionId
    )
