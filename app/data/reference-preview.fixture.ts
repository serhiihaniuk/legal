...
export async function validateReferencePreviewFixture() {
  ...
  assert(draft, "No draft provision preview was found")
  assert(draft.sourceEditionId, "Draft preview has no source edition metadata")
  assert(
    draft.sourceEditionId === draft.editionId,
    "Draft preview editorial content must match the requested edition"
  )
  assert(draft.verifiedAt, "Draft preview has no verification metadata")
  assert(draft.legalStateDate, "Draft preview has no legal-state metadata")
  assert(
    draft.reviewStatus && draft.reviewStatus !== "reviewed",
    "Draft preview must retain its non-reviewed editorial status"
  )

  const art114 = provisionPreviews.find(
    (preview) =>
      preview.documentId === "ustawa-o-cudzoziemcach" &&
      preview.provisionId === "ustawa-o-cudzoziemcach-art-114"
  )
  assert(
    art114?.status === "draft",
    "Art. 114 must use its draft editorial summary"
  )

  const sourceOnly = provisionPreviews.find(
    (preview) => preview.status === "source-only"
  )
  assert(sourceOnly, "No source-only provision preview was found")
  assert(sourceOnly.sourceTextExcerpt, "Source-only provision has no excerpt")
  assert(
    !sourceOnly.reviewStatus,
    "Source-only preview must have no editorial status"
  )
  assert(
    sourceOnly.summary.includes(sourceOnly.locator ?? ""),
    "Source-only preview does not preserve the exact locator"
  )
  ...
  return Object.freeze({
    ...
    sourceOnly: sourceOnly.identity,
  })
}
